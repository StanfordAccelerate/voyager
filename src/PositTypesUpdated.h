...
PositFP<sbits, fbits>::operator+(const PositFP<sbits, fbits> &op) const {
  ...
  ac_int<8, true> lhs_scale = lhs.scale;
  ac_int<8, true> rhs_scale = rhs.scale;
  ac_int<8, true> scale_of_result = max(lhs_scale, rhs_scale);
  ...
  // align the fraction
  ac_int<abits, false> r1 = lhs_fraction >> (scale_of_result - lhs_scale);
  r1[0] = (lhs_fraction << (abits - (scale_of_result - lhs_scale) + 1))
              .or_reduce();  // sticky bit
  ac_int<abits, false> r2 = rhs_fraction >> (scale_of_result - rhs_scale);
  r2[0] = (rhs_fraction << (abits - (scale_of_result - rhs_scale) + 1))
              .or_reduce();  // sticky bit

  bool lhs_lt_rhs =
      lhs_scale < rhs_scale || (lhs_scale == rhs_scale && r1 < r2);

  if (signs_are_different && lhs_lt_rhs) {
    swap_(r1, r2);
    swap_(r1_sign, r2_sign);
  }

  // if (signs_are_different) {
  //   r2 = r2.bit_complement() + 1;
  // }
  // ac_int<abits + 1, false> sum = r1 + r2;
  // sum[abits] &= ~signs_are_different;
...
PositFP<sbits, PositFP<sbits, fbits>::mbits> PositFP<sbits, fbits>::operator*(
  ...
  product.scale = lhs.scale + rhs.scale;
  product.fraction = lhs.get_fixed_point() * rhs.get_fixed_point();

  if (product.fraction[mbits - 1]) {
    product.scale++;
    product.fraction <<= 1;
  } else {
    product.fraction <<= 2;
...
Posit<nbits2, es2> fma(const Posit<nbits, es> &a, const Posit<nbits, es> &b,
  ...
  constexpr size_t fhbits2 = fbits2 + 1;
  constexpr size_t abits = fhbits2 + 3;  // size of the addend

  PositFP<8, fbits> va(a);
  PositFP<8, fbits> vb(b);
