...
$(CODEGEN_DIR)/networks/llama_decode_kivi/%/model.txt: quantized-training/test/test_codegen.py
	mkdir -p $(dir $@)
	python quantized-training/test/test_codegen.py llm_kivi $($(notdir $(patsubst %/,%,$(dir $@)))_FLAGS) $(EXTRA_COMPILER_FLAGS) --model_output_dir $(dir $@) $(COMMON_FLAGS) $(LLM_FLAGS) &> $(dir $@)codegen.log
...
$(CODEGEN_DIR)/networks/llama_prefill_spmm/%/model.txt: quantized-training/test/test_codegen.py
	mkdir -p $(dir $@)
	python quantized-training/test/test_codegen.py llm_prefill $($(notdir $(patsubst %/,%,$(dir $@)))_FLAGS) $(EXTRA_COMPILER_FLAGS) --model_output_dir $(dir $@) $(COMMON_FLAGS) $(LLM_FLAGS) --mixed_precision --outlier 6.0 &> $(dir $@)codegen.log
