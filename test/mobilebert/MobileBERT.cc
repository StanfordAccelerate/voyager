...
std::vector<Workload> MobileBERT::getWorkloads(
  ...
  for (const std::string& layer : layersInRange) {
    ...
    workload.files = testFiles.at(layer);
    ...
    if (!workload.files.inputs_file.empty()) {
      workload.files.inputs_file.insert(0,
                                        dataDir + inputDataDir + encLayerName);
    }
    ...
    workload.files.outputs_file.insert(0,
                                       dataDir + outputDataDir + encLayerName);
    workload.files.bias_file.insert(0, dataDir + weightDataDir + encLayerName);
